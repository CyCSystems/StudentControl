 ...
 * @author angel
 */
public class JornadasForm extends javax.swing.JInternalFrame {

    /**
     * Creates new form Jornadas
     */
    private Coordinador coordinador;
    private Boolean ModificandoJornada = false;
    ...
    private void initComponents() {
        ...
        setTitle("Administracion de Jornadas");
    ...
    private void btnNuevoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnNuevoActionPerformed
        ...
        btnModificar.setEnabled(false);
        tblJornadas.setEnabled(false);
    }//GEN-LAST:event_btnNuevoActionPerformed

    private void btnGuardarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnGuardarActionPerformed
        if(!txtJornada.getText().trim().equals("") && cbEstado.getSelectedIndex() != -1){
            Jornada jornada = new Jornada();
            boolean Resultado = false;
            jornada.setIdJornada(Integer.parseInt("".equals(txtCodigo.getText()) ? "0" : txtCodigo.getText()));
            jornada.setDescripcion(txtJornada.getText());
            jornada.setEstado((cbEstado.getSelectedIndex() == 0));
            if(ModificandoJornada == false){
                if(cbEstado.getSelectedIndex() != -1){                
                    if(coordinador.registrarJornada(jornada) == true){
                       Resultado = true;
                    }else{
                        JOptionPane.showMessageDialog(null, "Error: Guardando Jornada intente de nuevo\nsí persiste el error comuniquese con el administrador", "Student Control", JOptionPane.ERROR_MESSAGE);
                    }
                }else{
                    Resultado = false;
                    JOptionPane.showMessageDialog(null, "Error: Debe elegir el estado de la jornada", "Student Control", JOptionPane.ERROR_MESSAGE);
                }
            }else if(ModificandoJornada == true){
                if(coordinador.modificarJornada(jornada) == true){
                    Resultado = true;
                }else{
                    Resultado = false;
                    JOptionPane.showMessageDialog(null, "Error: Guardando Jornada intente de nuevo\nsí persiste el error comuniquese con el administrador", "Student Control", JOptionPane.ERROR_MESSAGE);
            ...
            if(Resultado == true){
                 JOptionPane.showMessageDialog(null, "Jornada guardada exitosamente", "Student Control", JOptionPane.INFORMATION_MESSAGE);
                desactivarCampos();
                limpiarCampos();
                coordinador.cargarTblJornadas();
                btnGuardar.setEnabled(false);
                btnCancelar.setEnabled(false);
                btnNuevo.setEnabled(true);
                btnModificar.setEnabled(false);
                tblJornadas.setEnabled(true);
                ModificandoJornada = false;
            }
        }else if(txtJornada.getText().trim().equals("")){
            JOptionPane.showMessageDialog(null, "Debe especificar el nombre de la jornada", "Student Control", JOptionPane.ERROR_MESSAGE);
        }else if(cbEstado.getSelectedIndex() == -1){
            JOptionPane.showMessageDialog(null, "Debe seleccionar el estado de la jornada", "Student Control", JOptionPane.ERROR_MESSAGE);
        }
    ...
    private void txtJornadaKeyTyped(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtJornadaKeyTyped
    ...
    }//GEN-LAST:event_txtJornadaKeyTyped

    private void tblJornadasMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblJornadasMouseClicked
        int row = tblJornadas.getSelectedRow();
        txtCodigo.setText(tblJornadas.getValueAt(row, 0).toString());
        txtJornada.setText(tblJornadas.getValueAt(row, 1).toString());
        cbEstado.setSelectedIndex(tblJornadas.getValueAt(row, 2) == "Activo" ? 0 : 1);
        btnGuardar.setEnabled(false);
        btnCancelar.setEnabled(true);
        btnNuevo.setEnabled(false);
    ...
    public javax.swing.JTable tblJornadas;
